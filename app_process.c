...
void app_init(){
  ...
  emberAfAllocateEvent(&Instalation_control, &Instalation_handler);
...
void Instalation_handler(){
  ...
  emberEventControlSetInactive(*Instalation_control);
}
...
void app_button_press_cb(uint8_t button, uint8_t duration)
{
  if(button == 0){
      ...
      if(duration >= 3){
          application.radio.LastCMD = STATUS_CENTRAL;
  ...
  uint8_t tamper_state =  GPIO_PinInGet(gpioPortA, 10);
   ...
   switch (application.Status_Operation) {
     ...
     case PERIOD_INSTALATION:
       ...
       if(application.radio.LastCMD == SETUP_LR){
           sendRadio.cmd = SETUP_LR;
       }
     ...
     case BOOT:
       if(waiting_registration){
           ...
           if(joined){
               if(just_joined){
                   application.Status_Operation = PERIOD_INSTALATION;
                   emberAfPluginPollEnableShortPolling(true);
               }else{
                   application.Status_Operation = OPERATION_MODE;
                   emberAfPluginPollEnableShortPolling(false);
       ...
       }else if(application.Status_Operation == PERIOD_INSTALATION){
           application.IVP.SensorStatus.Status.led_enabled = Detection_only;
           application.IVP.SensorStatus.Status.energy_mode = CONTINUOUS;
